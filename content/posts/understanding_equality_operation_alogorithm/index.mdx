...
because its easy to write. But it just means *makeSambar is an operation that somehow makes* <a href="https://en.wikipedia.org/wiki/Sambar_(dish)" target="_blank">Sambar</a> *for us*

So when you  look up the spec for a JS function, (e.g) <a href="https://www.indianhealthyrecipes.com/Sambar-sadam-Sambar-rice/" target="_blank">Food.Prototype.makeSambarSaadham()</a>

the algorithm might look something like this.


* let *r* be the result of `makeSaadham(Rice)`.
* let *s* be the result of `makeSambar(dhaal, onion, SambarPowder, corianderLeaves)`.
* `return` `mixRiceAndSaadham(r,s)`


in the above algorithm, `makeSaadham`, `makeSambar` and `mixRiceAndSaadham` are abstract operations which are not
of interest in the current algorithm as long as we got saadham and Sambar and mix them together properly. These
operations might be implemented differently in each JS Engine, based on a lot of other things. But the spec guarantee
that when you call `makeSambarSaadham()`, we first make the saadham, then the Sambar and then mix them together and
...
* make Saadham first using rice.
* make Sambar using dhaal, onion, SambarPowder, corianderLeaves.
* return a good bowl of saadham and Sambar mixed into a Sambar Saadham.

now ask yourself.

* does it matter if the Sambar is cooked on a electric stove or a gas stove or just with a bunch of coal and fire?
